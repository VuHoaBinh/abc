...
const addToCart = function (name, price) {
  ...
  if (name == null && price == null) return; //if theres no name and price, return
  const existingItem = cartItems.find((item) => item.name === name);
  if (!existingItem) {
    cartItems.push({ name, price }); //if the item exists, push it into the card (record it into the cart)
